...
class ActorNet:
    ...
    def __init__(self,num_states,num_actions):
        ...
        with self.g.as_default():          
            ...
            rate = tf.train.exponential_decay(LEARNING_RATE,global_step,100,0.8,staircase=True)
...
# state와 action. 정보가 4개 골까지의 거리, 라이다에서 나온 최소거리, 왼쪽 바퀴 속도, 오른쪽 바퀴 속도. , 움직임이 2개 : 각도와 속도
    def create_actor_net(self, num_states=38, num_actions=1):
        ...
        H2_a=tf.nn.relu(tf.matmul(H1_a,W2_a)+B2_a)
    ...
    def save_model(self):
        ...
        self.saver.save(self.sess, './model/save/actor_model.ckpt')

    def load_model(self):
        try:
            self.saver.restore(self.sess, './model/load/actor_model.ckpt')
        except:
            pass
